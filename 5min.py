...
def indicator(symbol):   
      ...
      #Tendencia FISHING
        ...
        #Tendencia view
        if (cciBTC[-2] < 0) and (df['EMA200'][-2] > Close) and (float(df['Close'][-2]) < float(df['third_level'][-2])) and (float(df['third_level'][-3])) < (float(df['Close'][-3])) and (adx[-2] >= 40):
         Tb.telegram_canal_prueba(f"⚡️ {symbol}\n🔴 SHORT\n⏳ 5 min\n💵 Precio: {Close}\n⛳️ Snipper : {max_ask} \n📕 Trend Call") 
         requests.post('https://hook.finandy.com/gZZtqWYCtUdF0WwyqFUK', json=VIEWSHORT) 
        if (cciBTC[-2] > 0) and (df['EMA200'][-2] < Close) and (float(df['Close'][-2]) > float(df['third_level'][-2])) and (float(df['third_level'][-3])) > (float(df['Close'][-3])) and (adx[-2] <= 20):
          Tb.telegram_canal_prueba(f"⚡️ {symbol}\n🟢 LONG\n⏳ 5 min\n💵 Precio: {Close}\n⛳️ Snipper : {max_bid} \n📗 Trend Call") 

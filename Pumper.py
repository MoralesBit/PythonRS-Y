...
def indicator(symbol):
  ...
  if (rsi[-2:].values[0] >= 25) and (rsi[-1:].values[0] < 25) and (mfi[-2] >= 20) and (mfi[-1] < 20) and (adx[-2] <= 23) and (adx[-1] > 23):
    requests.post('https://hook.finandy.com/yOiR__CztpkFLaRKqFUK', json=CORTO)
    Tb.telegram_send_message(" 🔴 SHORT  " + symbol + "\n 💵 Precio: " + df['Close'][-1])
  elif (rsi[-2:].values[0] <= 75) and (rsi[-1:].values[0] > 75) and (mfi[-2] <= 80) and (mfi[-1] > 80) and (adx[-2] <= 23) and (adx[-1] > 23):
